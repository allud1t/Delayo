...
function TimePicker({
  ...
  const minuteOptions = MINUTES.includes(safeMinute)
    ? MINUTES
    : [...MINUTES, safeMinute].sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      <div className='flex items-center gap-2'>
        <div className='flex items-center gap-1.5 rounded-lg bg-base-100/70 p-1.5 shadow-sm'>
          <select
            className={`select select-bordered select-sm border-none bg-base-200 font-semibold focus:outline-none ${isPopup ? 'h-9 text-sm' : ''}`}
            value={safeHour}
        ...
        <div className='flex flex-wrap gap-1'>
          {QUICK_TIMES.map((qt) => (
            <button
              key={qt.value}
              type='button'
              className={`btn btn-xs ${value === qt.value ? 'btn-primary' : 'btn-ghost bg-base-100/50'}`}
